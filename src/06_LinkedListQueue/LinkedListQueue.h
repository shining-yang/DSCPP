...
template <typename T> void LinkedListQueue<T>::Output(ostream &os) const {
...
template <typename T> bool LinkedListQueue<T>::IsEmpty() const {
  return DoublyLinkedCircularList<T>::IsEmpty();
}

template <typename T> bool LinkedListQueue<T>::IsFull() const { return false; }

template <typename T> int LinkedListQueue<T>::Length() const {
  return DoublyLinkedCircularList<T>::Length();
}

template <typename T> T LinkedListQueue<T>::Front() const {
  if (IsEmpty()) {
...
template <typename T> T LinkedListQueue<T>::Rear() const {
  if (IsEmpty()) {
    throw new OutOfBounds;
  }
...
template <typename T> LinkedListQueue<T> &LinkedListQueue<T>::DeQueue(T &x) {
