...
template <typename T> class DoublyLinkedCircularListNode {
private:
...
protected:
  ...
  operator=(const DoublyLinkedCircularList<T> &obj);
...
public:
  ...
  DoublyLinkedCircularList<T> &Delete(int k,
                                      T &x); // valid k: [0, Length() - 1]
  DoublyLinkedCircularList<T> &Insert(int k,
                                      const T &x); // valid k: [0, Length() - 1]
...
template <typename T> void DoublyLinkedCircularList<T>::_CreateHeadNode() {
  head = new DoublyLinkedCircularListNode<T>;
}

template <typename T> void DoublyLinkedCircularList<T>::_DestroyHeadNode() {
  delete head;
}

template <typename T> void DoublyLinkedCircularList<T>::_Init() {
  head->prev = head;
  head->next = head;
}

template <typename T> void DoublyLinkedCircularList<T>::_ClearNodes() {
...
template <typename T> DoublyLinkedCircularList<T>::DoublyLinkedCircularList() {
  _CreateHeadNode();
  _Init();
}
...
template <typename T> void DoublyLinkedCircularList<T>::Clear() {
  _ClearNodes();
  _Init();
}

template <typename T> int DoublyLinkedCircularList<T>::Length() const {
  int n = 0;
  for (DoublyLinkedCircularListNode<T> *p = head->next; p != head;
       p = p->next) {
    n++;
  }
...
template <typename T> bool DoublyLinkedCircularList<T>::IsEmpty() const {
...
bool DoublyLinkedCircularList<T>::FindReversely(int k, T &x) const {
  ...
  int n = 0;
  for (DoublyLinkedCircularListNode<T> *p = head->prev; p != head;
       p = p->prev, n++) {
    if (k == n) {
      x = p->data;
      return true;
...
template <typename T>
DoublyLinkedCircularListNode<T> *
DoublyLinkedCircularList<T>::_LocateNode(int k) const {
...
template <typename T>
void DoublyLinkedCircularList<T>::_DeleteNode(
    DoublyLinkedCircularListNode<T> *p, T &x) {
  if (p) {
    p->prev->next = p->next;
    p->next->prev = p->prev;
    x = p->data;
...
template <typename T>
void DoublyLinkedCircularList<T>::_InsertNode(
    DoublyLinkedCircularListNode<T> *p, const T &x) {
  if (p) {
...
template <typename T>
DoublyLinkedCircularList<T> &DoublyLinkedCircularList<T>::Insert(int k,
                                                                 const T &x) {
  DoublyLinkedCircularListNode<T> *p = _LocateNode(k);
  _InsertNode(p, x);
  return *this;
