...
template <class T> class Term {
  ...
  template <class E>
  friend ostream &operator<<(ostream &, const SparseMatrix<E> &);
...
template <class T> istream &operator>>(istream &is, SparseMatrix<T> &m) {
...
template <class T> ostream &operator<<(ostream &os, const SparseMatrix<T> &m) {
  ...
  for (int i = 0; i < m.size; i++) {
    ...
    os << "[" << m.elements[i].r << "," << m.elements[i].c << "] "
       << m.elements[i].v;
  }
...
template <class T> SparseMatrix<T>::SparseMatrix(int c /*= 16*/) {
  if (c < 1) {
...
template <class T> SparseMatrix<T>::~SparseMatrix() { delete[] elements; }
...
template <class T> void SparseMatrix<T>::Output(ostream &os) const {
...
template <class T> void SparseMatrix<T>::Transpose(SparseMatrix<T> &dm) const {
...
template <class T> void SparseMatrix<T>::_Append(const Term<T> &e) {
  if (size >= capacity) {
...
template <class T>
void SparseMatrix<T>::Add(const SparseMatrix<T> &sm,
                          SparseMatrix<T> &dm) const {
