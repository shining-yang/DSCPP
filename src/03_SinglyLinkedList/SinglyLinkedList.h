...
bool Chain<T>::Bubble(int n, bool ascending /*= true*/)
{
    ...
    while (n-- > 1) { // require 2 nodes at least
        ...
        if ((p->data > q->data && ascending) ||
            (p->data < q->data && !ascending)) {
            swap_required = true;
        }
...
void Chain<T>::Insert(ChainNode<T>* t, bool ascending /*= true*/)
{
    ...
    while (p) {
        if ((t->data < p->data && ascending) ||
            (t->data > p->data && !ascending)) {
            t->link = p;
            if (p == this->first) {
                this->first = t;
            } else {
...
template<class T>
void Chain<T>::InsertionSort(bool ascending /*= true*/)
{
    ChainNode<T>* p = first;
    first = NULL;
    while (p) {
...
template<class T>
void Chain<T>::RankSort(bool ascending /*= true*/)
{
    int n = Length();
    if (n < 2) {
        return;
...
template<class T>
void Chain<T>::SelectionSort(bool ascending /*= true*/)
{
    ChainNode<T>* nl = NULL;
...
//
template<class T>
void ReverseChain(Chain<T>& a, Chain<T>& b)
{
    T x;
    while (!a.IsEmpty()) {
        a.Delete(0, x);
