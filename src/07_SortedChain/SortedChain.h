...
template <typename E, typename K> SortedChain<E, K>::~SortedChain() {
  while (first) {
...
template <typename E, typename K> bool SortedChain<E, K>::IsEmpty() const {
  return first == NULL;
}

template <typename E, typename K> int SortedChain<E, K>::Length() const {
  int n = 0;
