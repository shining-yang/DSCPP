...
template <typename T> class LinearStack : private LinearList<T> {
public:
  enum { DEFAULT_STACK_SIZE = 20 };
...
template <typename T> LinearStack<T>::LinearStack(int c) : LinearList<T>(c) {}

template <typename T> LinearStack<T>::~LinearStack() {}

template <typename T> bool LinearStack<T>::IsEmpty() {
  return LinearList<T>::IsEmpty();
}

template <typename T> bool LinearStack<T>::IsFull() {
  return LinearList<T>::GetMaxSize() == LinearList<T>::Length();
}

template <typename T> LinearStack<T> &LinearStack<T>::Push(const T &e) {
  LinearList<T>::Insert(LinearList<T>::Length(), e);
  return *this;
}

template <typename T> LinearStack<T> &LinearStack<T>::Pop(T &e) {
  LinearList<T>::Delete(LinearList<T>::Length() - 1, e);
  return *this;
}

template <typename T> T LinearStack<T>::Top() const {
  if (IsEmpty()) {
...
//==============================================================================
template <typename T> class LinearStack2 {
public:
  enum { DEFAULT_STACK_SIZE = 20 };
